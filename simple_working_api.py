#!/usr/bin/env python3
"""
Simple working API server using the same approach as test_working_script.py
"""
import os
import cv2
import numpy as np
...
def load_model():
    """Load the simclr model using the same approach as test_working_script.py"""
    global model
    try:
...
def analyze():
    """Analyze wound image using the same approach as test_working_script.py"""
    try:
        ...
        # Convert to RGB and resize (same as working script)
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img_resized = cv2.resize(img_rgb, (128, 128))
        img_normalized = img_resized.astype(np.float32) / 255.0
        img_batch = np.expand_dims(img_normalized, axis=0)
        
        # Predict with model (same as working script)
        if model is None:
        ...
        # Calculate metrics (same as working script)
        ...
        # Create visualization (same as working script)
...
if __name__ == '__main__':
    print("🚀 Starting Simple Working Wound Detection API")
    print("=" * 50)
